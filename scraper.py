...
def _extract_canonical_url(google_news_link: str) -> str:
    # Ambil url kanonis dari parameter ?url= jika ada
    try:
...
def scrape_news(query: str, max_results: int = 10) -> pd.DataFrame:
    """
    Scraper tunggal berbasis RSS Google News.
    Output kolom: title, summary, url
    """
    ...
    try:
        ...
        print(f"❌ Gagal mengunduh RSS: {e}")
        return pd.DataFrame(columns=["title", "summary", "url"])

    soup = BeautifulSoup(resp.content, "xml")  # butuh lxml
